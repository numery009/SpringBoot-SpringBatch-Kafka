...
public class Demo1 {
	...
	public Step step1Demo() throws Exception {
		return this.stepBuilderFactory.get("step1").<EmployeeDTO, Employee>chunk(5).reader(employeeReader())
				.processor(employeeProcessor).writer(employeeDOWriterDefault()).listener(readerListern())
				.listener(processListener()).listener(writerListener()).faultTolerant().skipPolicy(skipPolicy())
				.build();
				...
				setLineTokenizer(new FixedLengthTokenizer() {
					{
						...
						setColumns(new Range[] { new Range(1, 4), new Range(5, 13), new Range(14, 21),
								new Range(22, 37), new Range(38, 40) });
						setStrict(false);
					}
				});
